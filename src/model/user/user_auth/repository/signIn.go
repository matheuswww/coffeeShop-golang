...
func (ur *userAuthRepository) SignIn(userDomain user_auth_model.UserAuthDomainInterface) *rest_err.RestErr {
	...
	defer cancel()
	logger.Info("Init SignIn repository", zap.String("journey", "SignIn"))
	...
	if err != nil {
			logger.Error("Error trying SignIn user", err, zap.String("journey", "SignIn"))
	...
	if result.Next() {
		if err = result.Scan(&encryptedPassword, &salt); err != nil {
			logger.Error("Error scanning result", err, zap.String("journey", "SignIn"))
			return rest_err.NewInternalServerError("database error")
		}
	} else {
		logger.Error("Error email or password not found", err, zap.String("journey", "SignIn"))
		return rest_err.NewUnauthorizeError("Email not registred")
	}
	encrypt_err := encryptUserPasswordWithSalt(userDomain,salt)
	if encrypt_err != nil {
		logger.Error("Error trying encrypt Password", err, zap.String("journey", "SignIn"))
		return rest_err.NewInternalServerError("server error")
	}
	if subtle.ConstantTimeCompare(userDomain.GetEncryptedPassword(),encryptedPassword) != 1 {
		logger.Error("Incorrect password or email", err, zap.String("journey", "SignIn"))
...
func encryptUserPasswordWithSalt(userDomain user_auth_model.UserAuthDomainInterface,salt []byte) *rest_err.RestErr{
	...
	if err != nil {
		logger.Error("Error tryin encrypt password",err,zap.String("journey","SigIn"))
		return rest_err.NewInternalServerError("database error")
	}
