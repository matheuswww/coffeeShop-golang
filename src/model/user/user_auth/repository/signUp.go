...
func (ur userAuthRepository) SignUp(userAuthDomain user_auth_model.UserAuthDomainInterface) *rest_err.RestErr {
	...
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			logger.Error("Timeout expired",err,zap.String("journey","SignUp"))
		} else {
			logger.Error("Error trying exec query",err,zap.String("journey","SignUp"))
		}
	...
	if rows.Next() {
		...
		if err != nil {
			logger.Error("Error trying scan query",err,zap.String("journey","SignUp"))
			return rest_err.NewInternalServerError("database error")
		}
		logger.Error("Error trying insert user", errors.New("duplicated email"), zap.String("journey", "SignUp"))
	...
	if err != nil {
		logger.Error("Error trying get id",err,zap.String("journey","SignUp"))
		return rest_err.NewInternalServerError("database error")
	}
